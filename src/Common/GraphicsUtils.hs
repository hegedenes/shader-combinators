{-# LANGUAGE OverloadedStrings, NamedFieldPuns, ParallelListComp, DataKinds, FlexibleContexts, TypeOperators, ViewPatterns #-}
...
getDist (unpack' -> V3 ax ay az) (unpack' -> V3 bx by bz)
...
v2v4 v = pack' (V4 x y (Const 0) (Const 1))
  where
    V2 x y = unpack' v
...
denesCamera :: Vec3 -> Signal Vec2 -> Signal (Bool, Bool, Bool, Bool, Bool) -> SignalGen Float (Signal (Vec3, Vec3, Vec3, Vec2, Vec2))
denesCamera startPosition mouseDelta directionKeys = transfer2 (startPosition, zero, zero, zero, zero) calcCam mouseDelta directionKeys
  where
    dir0 = Vec4 0 0 (-1) 1
    up0 = Vec4 0 1 0 1
    calcCam dt dm (ka, kw, ks, kd, turbo) (pos0, _, _, m, Vec2 camU camV) = (pos', dir, up, m', Vec2 newU newV)
      where
        f0 c v = if c then v else zero
        pos' = pos0 &+ (f0 kw dir &- f0 ks dir &+ f0 kd v &- f0 ka v) &* (realToFrac dt * if turbo then 5 else 1)
        m' = dm &+ m
        Vec2 dmX dmY = dm
        newU = camU + (dmX / 100.0)
        newV = clamp (camV + (dmY / 100.0)) 0.1 3.1
        rm = fromProjective $ rotationEuler $ extendZero (m' &* 0.01)
        dir  = Vec3 (cos newU * sin newV) (cos newV) (sin newU * sin newV)
